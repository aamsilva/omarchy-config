...
- **opencodemini** (`~/.local/share/applications/opencodemini.desktop` → `/home/aamsilva/bin/opencodemini`): abre `opencode attach http://100.74.228.17:4096 -p <pass>` (servidor opencode no Mac Mini augustosilva via Tailscale, porta 4096, sem `--token`, pass em `~/.config/opencode/.secrets.env` no remoto). Menu Omarchy lê DesktopEntries. NOTA: `omarchy-launch-or-focus-tui` exige ARGUMENTOS SEPARADOS, não string única — usar wrapper script. **BUG FIXADO (03/09):** `Exec=~/bin/opencodemini` com `~` inválido tornava o `.desktop` INVALIDO (`gtk-launch` → "no such application"), não abria janela. FIX: `Exec=/home/aamsilva/bin/opencodemini` (caminho absoluto, sem `~`). Menu "Apps" lança DESKTOP ENTRIES via `gtk-launch <id>.desktop` (não via launch-or-focus-tui), que NÃO expande `~` nem define --app-id. Ao criar `.desktop` usar sempre caminho absoluto no Exec.
...
11. **Nuvio (nuvio-desktop-bin, AUR, alpha 0.1.22)** instalado + reinstalado/testado em 04/09. App de media open-source (GPLv3) do NuvioMedia. **Lições:** (a) o player embutido (JVM/Compose, `composeMediaPlayer`/`applauncher`) NÃO ativa hwdecode VAAPI → CPU ~80% em reproduzir 4K HEVC/HDR; (b) o `[nuvio-bridge] snapshot stuck in flight >30 ticks, resetting` só aparece se forçares X11 (`DISPLAY=:0`) — em **Wayland nativo o bridge não encrava**; (c) FIX p/ playback pesado/4K/IPTV live: usar o mpv do sistema por fora via **`~/bin/nuvio-play`** (wrapper: `mpv --vo=gpu-next --hwdec=vaapi-copy --vaapi-device=/dev/dri/renderD128 ["URL"|magnet|ficheiro]`); (d) a RX580 faz HW decode via VAAPI (`vainfo` de `libva-utils`): HEVCMain, HEVCMain10(10-bit/HDR), H264High, MPEG2/VC1/JPEG; (e) o `mpv` v0.41 tem `vaapi-wayland` compilado e abre `radeonsi_drv_video.so`; (f) login Nuvio pede **credenciais completas** da conta (não só email) — `aamsilva@gmail.com` sozinho deu `AuthRestException: invalid_credentials`.
...
- USB autosuspend no rato/teclado (lag 1º movimento): `echo on > /sys/bus/usb/devices/1-5/power/control` e `1-7` (receivers Logitech) + `udevadm trigger --subsystem-match=usb` para re-aplicar regras sem replug. **DONGLE WiFi** rule reposta: `/etc/udev/rules.d/90-usb-wifi-no-autosuspend.rules` (`ATTR{idVendor}=="0bda", ATTR{idProduct}=="b851"` → `power/control=on`) para `3-1` persistir no boot.
- **I/O scheduler: MANTER `mq-deadline` no SATA TR200** — não usar `bfq` (sem sintoma atual de freeze UI; disco de sistema em LUKS/btrfs dilui o ganho; `bfq` só se surgirem freezes de UI sob I/O pesado).
- **`omarchy update` pode exceder 120s (pacman + gpg-agent)** — não interromper; esperar/fazer em background com log. Verificar depois que plugins/eestados ok.
- `librepods` pode estar `inactive (dead)` sem problema — só precisa `systemctl --user start librepods` se quiseres AirPods a correr.
- **A CAUSA RAIZ do librepods inactive (corrigido 03/09):** daemon morreu em 28/08 numa desconexão dos pods (exit 0, limpa) e o `Restart=on-failure` da unit NÃO o reinicia (só cobre falhas com status de erro). FIX: drop-in `~/.local/share/systemd/user/librepods.service.d/10-restart-always.conf` com `Restart=always` + `RestartSec=5` — reativa em qualquer saída. O watchdog `omarchy-airpods-watchdog` (25/08) só reconecta pods via BlueZ, NÃO reinicia o daemon (lacuna que o `Restart=always` tapa).
